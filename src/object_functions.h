...
std::vector<int> mutau_analyzer::simple_boostedtau_cand(double tauPtCut, double tauEtaCut, int shift)
{
  ...
  for (int iTau = 0; iTau < nBoostedTau; iTau++)
  {
    ...
    //cout<< dau2.Pt() << "  " << dau2.Eta() << "  " << dau2.Phi() << "  " << dau2.Energy() << endl;
    bool kinematic = false;
...
std::vector<int> mutau_analyzer::getJetCand(int muIndex, int tauIndex)
{
  ...
  for (int iJet = 0; iJet < nJet; iJet++) // Loop over jets
  {
    ...
    double lepton2Phi = 0;
    double lepton2Eta = 0;
    lepton2Phi = tau_Phi->at(tauIndex);
    lepton2Eta = tau_Eta->at(tauIndex);
...
bool mutau_analyzer::thirdLeptonVeto_boosted(int muIndex, int tauIndex)
{
  std::vector<int> tmpCand;
  tmpCand.clear();
  int thirdLepIndex = -1;
  ...
  {
    deltaRm1 = delta_R(muPhi->at(muIndex), muEta->at(muIndex), elePhi->at(tmpCand[0]), eleEta->at(tmpCand[0]));
    deltaRm2 = delta_R(boostedTauPhi->at(tauIndex), boostedTauEta->at(tauIndex), elePhi->at(tmpCand[0]), eleEta->at(tmpCand[0]));
    if (deltaRm1 > 0.5 && deltaRm2 > 0.5)
    {
      return false;
    }
...
vector<int> mutau_analyzer::bJet_medium(int muIndex, int tauIndex)
{
  ...
  double lepton1Phi = muPhi->at(muIndex);
  double lepton1Eta = muEta->at(muIndex);
  double lepton2Phi = tau_Phi->at(tauIndex);
  double lepton2Eta = tau_Eta->at(tauIndex);
...
vector<int> mutau_analyzer::bJet_medium_boosted(int muIndex, int tauIndex)
{
  std::vector<int> tmpJetCand;
  tmpJetCand.clear();
  bool veto = true;
  ...
  double lepton1Phi = muPhi->at(muIndex);
  double lepton1Eta = muEta->at(muIndex);
  double lepton2Phi = boostedTauPhi->at(tauIndex);
  double lepton2Eta = boostedTauEta->at(tauIndex);
  double dr_jetEle = 0.0;
  double dr_jetTau = 0.0;
...
vector<int> mutau_analyzer::bJet_loose_boosted(int muIndex, int tauIndex)
{
  std::vector<int> tmpJetCand;
  tmpJetCand.clear();
  bool veto = true;
