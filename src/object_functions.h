...
std::vector<int> tautau_analyzer::getTauCand_noID(double tauPtCut, double tauEtaCut, int shift)
{
  ...
  for (int iTau = 0; iTau < nTau; iTau++)
  {
    ...
    bool trigger = false;
    if (dau2.Pt() > tauPtCut && fabs(dau2.Eta()) < tauEtaCut && tau_LeadChargedHadron_dz->at(iTau) < 0.2 && fabs(tau_Charge->at(iTau)) == 1)
      kinematic = true;
    ...
    if (tau_DecayMode->at(iTau) == 0 || tau_DecayMode->at(iTau) == 1 || tau_DecayMode->at(iTau) == 10 || tau_DecayMode->at(iTau) == 11)
      decayModeCut = true;
    if (tau_byVVVLooseDeepTau2017v2p1VSe->at(iTau) == 1 && tau_byVLooseDeepTau2017v2p1VSmu->at(iTau) == 1)
      tau_reject = true;
    if (tau_IDbits->at(iTau) >> 1 & 1 == 1)
      newDecayModeFinding = true;

    if (kinematic == true && decayModeCut == true && tau_reject == true && newDecayModeFinding == true && tauIsolation == true)
    {
...
void tautau_analyzer::assignTauIndices(vector<int> tauIndices, int &tau1, int &tau2)
{
  ...
  else
  {
    for (int ai = 0; ai < tauIndices.size(); ai++)
    {
      ...
      {
        if (tau_byVVVLooseDeepTau2017v2p1VSjet->at(tauIndices[ai]) == 1 && !(tau_byMediumDeepTau2017v2p1VSjet->at(tauIndices[ai]) == 1) && tau_byMediumDeepTau2017v2p1VSjet->at(tauIndices[i]) == 1 && tau_Pt->at(tauIndices[ai]) > tau_Pt->at(tauIndices[i]))
        {
          tau1 = tauIndices[ai];
          tau2 = tauIndices[i];
          return;
...
std::vector<int> tautau_analyzer::simple_tau_cand_2(double tauPtCut, double tauEtaCut, int shift)
{
  std::vector<int> tmpCand;
  tmpCand.clear();
  TLorentzVector dau2;
  ...
  for (int iTau = 0; iTau < nBoostedTau; iTau++)
  {
    ...
    if (dau2.Pt() > tauPtCut && fabs(dau2.Eta()) < tauEtaCut && boostedTauZImpact->at(iTau) < 0.2 && fabs(boostedTauCharge->at(iTau)) == 1)
      kinematic = true;
...
std::vector<int> tautau_analyzer::getJetCand(int tau1Index, int tau2Index)
{
  ...
  for (int iJet = 0; iJet < nJet; iJet++) // Loop over jets
  {
    ...
    double lepton2Phi = 0;
    double lepton2Eta = 0;
    lepton2Phi = tau_Phi->at(tau2Index);
    lepton2Eta = tau_Eta->at(tau2Index);
...
bool tautau_analyzer::thirdLeptonVeto_boosted(int tau1Index, int tau2Index)
{
  std::vector<int> tmpCand;
  tmpCand.clear();
  int thirdLepIndex = -1;
  ...
  if (tmpEleCand.size() > 0)
  {
    deltaRm1 = delta_R(boostedTauPhi->at(tau1Index), boostedTauEta->at(tau1Index), elePhi->at(tmpEleCand[0]), eleEta->at(tmpEleCand[0]));
    deltaRm2 = delta_R(boostedTauPhi->at(tau1Index), boostedTauEta->at(tau1Index), elePhi->at(tmpEleCand[0]), eleEta->at(tmpEleCand[0]));
    if (deltaRm1 > 0.5 || deltaRm2 > 0.5)
    {
      return false;
    }
  ...
  if (tmpCand.size() > 0)
  {
    deltaRm1 = delta_R(boostedTauPhi->at(tau1Index), boostedTauEta->at(tau1Index), muPhi->at(tmpCand[0]), muEta->at(tmpCand[0]));
    deltaRm2 = delta_R(boostedTauPhi->at(tau2Index), boostedTauEta->at(tau2Index), muPhi->at(tmpCand[0]), muEta->at(tmpCand[0]));
    if (deltaRm1 > 0.5 || deltaRm2 > 0.5)
    {
      return false;
    }
...
vector<int> tautau_analyzer::bJet_medium(int tau1Index, int tau2Index)
{
  ...
  double lepton1Phi = tau_Phi->at(tau1Index);
  double lepton1Eta = tau_Eta->at(tau1Index);
  double lepton2Phi = tau_Phi->at(tau2Index);
  double lepton2Eta = tau_Eta->at(tau2Index);
...
bool tautau_analyzer::passDiElectronVeto(int eleIndex)
{
  ...
  {
    bool kinematic = false;
    if (elePt->at(iEle) > 15 && fabs(eleEta->at(iEle)) < 2.5 && fabs(eleD0->at(iEle)) < 0.045 && fabs(eleDz->at(iEle)) < 0.2)
      kinematic = true;
    bool electronId = false;
    if (eleIDbit->at(iEle) >> 3 & 1 == 1)
...
std::vector<int> tautau_analyzer::getGenMu()
{
  ...
  for (int imc = 0; imc < nMC; imc++)
  {
    ...
    if ((genMatch2->at(imc) >> 2 & 1 == 1 || genMatch2->at(imc) >> 4 & 1 == 1) && fabs(mcPID->at(imc)) == 13)
    {
      count2++;
    }
  }
...
double tautau_analyzer::getScaleFactors(double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  double sf_htt_tau1 = 1.0;
  double sf_htt_tau2 = 1.0;
  double sf_tau1idSF_m = 1.0;
  double sf_tau2idSF_m = 1.0;
  double sf_tau1idSF_vvvl = 1.0;
  double sf_tau2idSF_vvvl = 1.0;
  double sf_fakeEle = 1.0;
  double sf_fakeMu = 1.0;
  double sf_fakeEleES = 1.0;
  double sf_fakeMuES = 1.0;
...
void tautau_analyzer::jetFakeUnc(string histNumber, int tau1Index, int tau2Index, bool isFakeBkg, float event_weight)
{
  ...
  double mt = TMass_F(my_tau2P4.Pt(), my_tau2P4.Phi(), my_metP4.Pt(), my_metP4.Phi());
  double mvis = (my_tau1P4 + my_tau2P4).M();

  double FF_nominal = FF_weights_withlpt.get_ff(my_tau1P4.Pt(), mt, mvis, 0, my_tau2P4.Pt(), my_metP4.Pt(), my_njets, 0, 0, 0, 0, TString(" "));

  for (int i = 0; i < 3; i++)
  {
    double FF_unc = FF_weights_withlpt.get_ff(my_tau1P4.Pt(), mt, mvis, 0, my_tau2P4.Pt(), my_metP4.Pt(), my_njets, 0, 0, 0, 0, TString(jetFakesUnc[i] + "_" + uncs));
...
TLorentzVector tautau_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
  ...
  if (genmatch >= 5)
    tauP4 = tauP4 * get_BinContent(h_tauesSF, taudm);
  else if ((genmatch == 1 || genmatch == 3) && taudm == 0)
    tauP4 = tauP4 * 1.003;
  else if ((genmatch == 1 || genmatch == 3) && tau2dm == 1)
    tauP4 = tauP4 * 1.036;
...
TLorentzVector tautau_analyzer::applyTau1ESCorrections(TLorentzVector tauP4, int tauIndex, int shift)
{
  ...
  if (genmatch >= 5)
    tauP4Corr = tauP4 * get_BinContent(h_tauesSF, taudm);
  else if ((genmatch == 1 || genmatch == 3) && taudm == 0)
    tauP4Corr = tauP4 * 1.003;
  else if ((genmatch == 1 || genmatch == 3) && taudm == 1)
    tauP4Corr = tauP4 * 1.036;
...
{
  double sf_taufesSF = 1.0;
  if (tau_DecayMode->at(tauIndex) == 0 && abs(tau_Eta->at(tauIndex)) <= 1.4)
    sf_taufesSF = h_taufesSF->Eval(1);
  if (tau_DecayMode->at(tauIndex) == 0 && abs(tau_Eta->at(tauIndex)) > 1.4)
    sf_taufesSF = h_taufesSF->Eval(3);
  if (tau_DecayMode->at(tauIndex) == 1 && abs(tau_Eta->at(tauIndex)) <= 1.4)
    sf_taufesSF = h_taufesSF->Eval(5);
  if (tau_DecayMode->at(tauIndex) == 1 && abs(tau_Eta->at(tauIndex)) > 1.4)
    sf_taufesSF = h_taufesSF->Eval(7);
  return sf_taufesSF;
}
...
int tautau_analyzer::getTau1Cand(double tauPtCut, double tauEtaCut, int shift)
{
  ...
  for (int iTau = 0; iTau < nTau; iTau++)
  {
    tmp_tau_p4.SetPtEtaPhiE(tau_Pt->at(iTau), tau_Eta->at(iTau), tau_Phi->at(iTau), tau_Energy->at(iTau));
    if (is_MC)
      dau1 = applyTau1ESCorrections(tmp_tau_p4, iTau, shift);
    else
      dau1.SetPtEtaPhiE(tau_Pt->at(iTau), tau_Eta->at(iTau), tau_Phi->at(iTau), tau_Energy->at(iTau));
    bool kinematic = false;
    bool tauId = false;
    bool decayModeCut = false;
    bool tauIsolation = false;
    ...
    bool tau_reject = false;
    bool trigger = false;
    if (dau1.Pt() > tauPtCut && fabs(dau1.Eta()) < tauEtaCut && tau_LeadChargedHadron_dz->at(iTau) < 0.2 && fabs(tau_Charge->at(iTau)) == 1)
      kinematic = true;
    if (tau_byMediumDeepTau2017v2p1VSjet->at(iTau) == 1)
      tauIsolation = true;
    if (tau_DecayMode->at(iTau) == 0 || tau_DecayMode->at(iTau) == 1 || tau_DecayMode->at(iTau) == 10 || tau_DecayMode->at(iTau) == 11)
...
int tautau_analyzer::getTau2Cand(double tauPtCut, double tauEtaCut, int tau1Index, int shift)
{
  ...
  for (int iTau = 0; iTau < nTau; iTau++)
  {
    ...
    tmp_taup4.SetPtEtaPhiE(tau_Pt->at(iTau), tau_Eta->at(iTau), tau_Phi->at(iTau), tau_Energy->at(iTau));
    if (is_MC)
